...
function Login() {
  ...
  function intent(sources) {
    ...
    var loginClick$ = sources.DOM.select('.btn-log-in').events('click').map(function (ev) {
      ...
      var request = sources.Auth.API.requestLogin;
      request.send = data;
      return request;
    });
    ...
    var signUpClick$ = sources.DOM.select('.btn-signup').events('click').map(function (ev) {
      ...
      var request = sources.Auth.API.requestCreate;
...
},{"@cycle/dom":13,"xstream":130}],134:[function(require,module,exports){
'use strict';
...
var _LoginDriver = require('./LoginDriver');

var _LoginDriver2 = _interopRequireDefault(_LoginDriver);
...
function model(sources, loginActions, quoteActions) {
  ...
  // initial state
  var data = sources.Auth.getData();

  // merge reponses and screen actions
  var mergedResponses$ = _xstream2.default.merge(loginActions.response$, quoteActions.response$, loginActions.screenActions$);

  // merge requests
  var actionsRequest$ = _xstream2.default.merge(loginActions.request$, quoteActions.request$);

  // build the data object to be send to the views
  var actions$ = mergedResponses$.map(function (ev) {
    if (ev.request) {
...
function main(sources) {
  // init components
  var quotes = (0, _Quotes2.default)();
  var login = (0, _Login2.default)();
  ...
  /* STATE definition */
  var userState = model(sources, loginActions, quoteActions);
