...
function renderLoggedIn(state) {
  return (0, _dom.div)('.logged-in-form', [(0, _dom.span)('.user-name', 'Welcome ' + state.username), (0, _dom.button)('.btn-log-out', 'log out')]);
}
...
function renderQuote(text, logged) {
  return (0, _dom.div)('.quote-container', [(0, _dom.h1)(text), (0, _dom.span)('.protected', logged ? 'protected quote' : ''), (0, _dom.button)('.btn-get-quote .pure-button', 'get a quote')]);
}
...
function view(userState) {
  ...
  .startWith({ text: 'Loading...', screen: 'welcome' }).map(function (_ref) {
    ...
    return (0, _dom.div)('.page', [renderQuote(text, logged), (0, _dom.div)('.login-container', [renderLoginSection(screen, userState)])]);
  });
}
...
function loginIntent(sources, $state) {
  ...
  var user$ = sources.DOM.select('.user-input').events('input').map(function (ev) {
    return ev.target.value;
  });

  var password$ = sources.DOM.select('.user-password').events('input').map(function (ev) {
    return ev.target.value;
  });
  ...
  var userPass$ = _xstream2.default.combine(user, pass);
  ...
  // response event which filter by the category
  var loginResponse$ = sources.HTTP.select('create-user').flatten();

  var screenActions$ = loginClick$;
...
function main(sources) {
  ...
  var quoteActions = quoteIntent(sources, $state);

  var loginActions = loginIntent(sources, $state);
